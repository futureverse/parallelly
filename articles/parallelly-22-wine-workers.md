...
Windows via Wine (\verb{<https://www.winehq.org/>}) on Linux and macOS.
This can be useful when we need to run R code or call R packages that
work only on MS Windows.
...
[`.libPaths()`](https://rdrr.io/r/base/libPaths.html) - is owned by the
users that installed R in Wine. This means that is *not* write protected
for most users and any package can be installed there.
