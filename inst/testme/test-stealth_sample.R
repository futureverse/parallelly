...
s1 <- parallelly:::stealth_sample(1:10, size = 5)
...
stopifnot(!identical(s1, s2)) # parallelly:::stealth_sample should produce different sequence for same seed due to internal set.seed

# 2. size = 0L case
x <- 1:10
s_empty <- parallelly:::stealth_sample(x, size = 0L)
stopifnot(length(s_empty) == 0L, identical(s_empty, x[integer(0)]))

# 3. length(x) == 1L case without error
# 3.1. size = 1L
x_single <- 5
s_single_1 <- parallelly:::stealth_sample(x_single, size = 1L)
stopifnot(length(s_single_1) == 1L, identical(s_single_1, x_single))

# 3.2. replace = TRUE, size > 1L
s_single_rep <- parallelly:::stealth_sample(x_single, size = 3L, replace = TRUE)
stopifnot(length(s_single_rep) == 3L, all(s_single_rep == x_single))

# 4. Error case: length(x) == 1L, replace = FALSE, size > 1L
res <- tryCatch({
  parallelly:::stealth_sample(5, size = 2L, replace = FALSE)
}, error = function(e) e)
...
parallelly:::stealth_sample(1:10, size = 5)
